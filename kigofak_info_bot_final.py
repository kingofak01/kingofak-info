...
CHANNEL_USERNAME = "@kingofak_info" 
...
async def show_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    logo_url = "https://i.ibb.co/sp40SyPN/logo.webp"
    telegram_link = "https://t.me/kingofak_info"
    ...
    await context.bot.send_message(
        ...
        text=(
            ...
            f"{welcome_msg}\n\n"
            f"{points_info}\n\n"
            f"📢 <b>Official Channel:</b> <a href='{telegram_link}'>Join Here</a>\n\n"
            f"{menu_prompt}"
        ),
...
async def show_subscription_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Official styled subscription prompt — user must join channel first."""
    ...
    join_text = get_text(user_id, 'btn_join')
    verify_text = get_text(user_id, 'btn_verify')

    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton(join_text, url=CHANNEL_LINK)],
        [InlineKeyboardButton(verify_text, callback_data='check_sub')]
    ])

    # Official styled message with link preview feel
    msg = (
        "🚨 <b>To use this bot you must join our official Telegram channel first.</b> 🚨

"
        f"👉 <a href='{CHANNEL_LINK}'>Join Official Channel — @{CHANNEL_USERNAME}</a>

"
        "After joining, press the ✅ Verify button below to get access."
    )

    msg_source = update.callback_query.message if update.callback_query else update.message
    await msg_source.reply_text(msg, reply_markup=keyboard, parse_mode=ParseMode.HTML)
    return
